...
BEGIN
	...
	*/
	...
	SET @_params = N'@verbose tinyint, @truncate_table bit, @has_staging_table bit';
...
if	( (select isnull(max(CollectionTimeUTC),''2023-01-01 00:00'') from dbo.sql_agent_jobs_all_servers) < dateadd(minute, -10, getutcdate()) )
begin
	exec dbo.usp_GetAllServerCollectedData 
			...
			IF @verbose > 0
				PRINT @_sql;
			EXEC sp_executesql @_sql, @_params, @verbose, @truncate_table, @has_staging_table;
		END
