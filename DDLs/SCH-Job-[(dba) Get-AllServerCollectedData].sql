...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'dbo.sql_agent_jobs_all_servers', 
		...
		@command=N'sqlcmd -E -b -S localhost -d DBA -Q "EXEC dbo.usp_wrapper_GetAllServerCollectedData @recipients = ''some_dba_mail_id@gmail.com'', @step_name = ''dbo.sql_agent_jobs_all_servers'', @verbose = 0;"', 
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'(dba) Get-AllServerCollectedData', 
		...
		--,@schedule_uid=N'f7a1ab3a-74a0-4bdd-bcbd-f9022bacaa94'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
